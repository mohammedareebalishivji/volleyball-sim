...
import type { System, Vec2, CameraPresetId, NetHeight, Signal, ServerType, CourtTarget, BlockPattern, ComboZone } from './types'
...
export const ZONE_POS: Record<number, Vec2> = {
  1: [3.25, -4.5],
  2: [3.25, -1.4],
  3: [0, -1.4],
  4: [-3.25, -1.4],
...
export const BACK_ZONES: Set<number> = new Set([1, 5, 6])

// Base lineup orders (position order = [Z4, Z3, Z2, Z1, Z6, Z5])
export const LINEUPS: Record<System, string[]> = {
  '5-1': ['OH1', 'MB1', 'OPP', 'S', 'MB2', 'OH2'],
  '6-2': ['OH1', 'MB1', 'S1', 'S2', 'MB2', 'OH2'],
}

export const ROLE_META: Record<string, { label: string; color: string; short: string }> = {
...
// Serve types
export const SERVE_TYPES: ServerType[] = [
...
export const SPIKE_TARGETS: CourtTarget[] = [
...
export const COMBO_ZONES: ComboZone[] = [
...
export const SERVE_TARGETS: CourtTarget[] = [
...
export const RECEIVE_TARGETS: CourtTarget[] = [
...
export const SPIKE_ZONE: { front: { label: string; x: number; z0: number; z1: number; color: string }; back: { label: string; x: number; z0: number; z1: number; color: string } } = {
...
export const COURT_GRID: { x: number; z: number; side: 'A' | 'B' }[] = [
  // Team A rows (z < 0): front (near net), mid, deep
  ...[3, 2, 1].map((row) => [-3, 0, 3].map((x) => ({ x, z: -1.5 * row - 0.75, side: 'A' as const }))),
  // Team B rows (z > 0)
  ...[1, 2, 3].map((row) => [-3, 0, 3].map((x) => ({ x, z: 1.5 * row - 0.75, side: 'B' as const }))),
].flat()

// Camera presets: {name, position, target}
export const CAMERA_PRESETS: Record<CameraPresetId, { name: string; pos: [number, number, number]; target: [number, number, number] }> = {
...
export const NET_HEIGHTS: Record<NetHeight, number> = { men: 2.43, women: 2.24 }
